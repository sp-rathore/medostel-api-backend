...
## User Master API - DETAILED SPECIFICATION

**Implementation Status:** ✅ COMPLETE (Phase 2-3)
**Version:** 1.0.0
**Last Updated:** March 3, 2026
**Test Coverage:** 47/47 endpoint tests passing (100%)

The User Master API provides comprehensive user management functionality with search, create, and update operations. All endpoints are fully validated and tested.

### Overview of User Master Endpoints

| # | Method | Endpoint | Purpose | Status |
|---|--------|----------|---------|--------|
| 5.1 | GET | `/api/v1/users/search` | Search user by email or mobile | ✅ Implemented |
| 5.2 | POST | `/api/v1/users` | Create new user | ✅ Implemented |
| 5.3 | PUT | `/api/v1/users/{userId}` | Update user by ID | ✅ Implemented |
...
**Purpose:** Search for a user by email address or mobile number
**Authentication:** Not required
**Method:** GET
**Status Code:** 200 (Found) or 200 with existsFlag=false (Not Found)
**Rate Limit:** 100 requests/minute

#### Description
Allows searching for existing users by either email or mobile number. Returns user details if found, otherwise returns existsFlag=false.

#### Query Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `emailId` | string | Conditional | User email address (max 255 chars). Either emailId OR mobileNumber must be provided |
| `mobileNumber` | integer | Conditional | User mobile number (10 digits: 1000000000-9999999999). Either emailId OR mobileNumber must be provided |
...
# Search with both parameters (OR logic)
curl -X GET "http://localhost:8000/api/v1/users/search?emailId=john@example.com&mobileNumber=9876543210"
```

#### Success Response (200 - User Found)
...
#### Not Found Response (200 - User Not Found)
...
**400 - Invalid Request (Missing both parameters)**
```json
{
...
**Purpose:** Create a new user with auto-generated userId
**Authentication:** Not required
**Method:** POST
...
#### Description
Creates a new user account with automatic user ID generation. Email and mobile number must be unique across the system. All timestamps are auto-generated by the system.
...
| `currentRole` | string | Yes | One of 8 roles | User role (ADMIN, DOCTOR, HOSPITAL, NURSE, PARTNER, PATIENT, RECEPTION, TECHNICIAN) |
| `emailId` | string | Yes | Valid email, unique | Email address (regex validated) |
| `mobileNumber` | integer | Yes | 10 digits (1000000000-9999999999), unique | Mobile number |
| `status` | string | No | Default: 'active' | Status (active, pending, deceased, inactive) |
| `organisation` | string | No | Max 255 chars | Organization/hospital name |
| `address1` | string | No | Max 255 chars | Primary address |
| `address2` | string | No | Max 255 chars | Secondary address |
| `stateId` | string | No | Max 10 chars | State identifier |
| `stateName` | string | No | Max 100 chars | State name |
| `districtId` | string | No | Max 10 chars | District identifier |
| `cityId` | string | No | Max 10 chars | City identifier |
| `cityName` | string | No | Max 100 chars | City name |
| `pinCode` | string | No | Max 10 chars | PIN code |
...
**409 - Email & Mobile Combination Exists**
```json
{
...
#### Description
Updates existing user information. At least one field (other than commentLog) must be provided for update. The commentLog field is required for audit trail purposes. The userId and createdDate fields are immutable and cannot be changed.
...
#### Request Body (Optional Fields - At least 1 required + commentLog)

| Field | Type | Mutable | Constraints | Description |
|-------|------|--------|-------------|-------------|
| `firstName` | string | Yes | Max 50 chars | User's first name |
| `lastName` | string | Yes | Max 50 chars | User's last name |
| `currentRole` | string | Yes | One of 8 roles | User role |
| `emailId` | string | Yes | Valid email, unique | Email address |
| `mobileNumber` | integer | Yes | 10 digits, unique | Mobile number |
| `status` | string | Yes | active/pending/deceased/inactive | User status |
| `organisation` | string | Yes | Max 255 chars | Organization name |
| `address1` | string | Yes | Max 255 chars | Primary address |
| `address2` | string | Yes | Max 255 chars | Secondary address |
| `stateId` | string | Yes | Max 10 chars | State identifier |
| `cityId` | string | Yes | Max 10 chars | City identifier |
| `pinCode` | string | Yes | Max 10 chars | PIN code |
| `commentLog` | string | Yes | Max 255 chars | **REQUIRED** - Reason for update (audit trail) |
...
- **Range:** 1000000000 to 9999999999 (10 digits exactly)
- **Type:** Integer
- **Uniqueness:** Must be unique across all users
- **Validation:** Range check in database + API layer
...
1. **Email Uniqueness:** emailId must be unique per user
2. **Mobile Uniqueness:** mobileNumber must be unique per user
3. **Composite Uniqueness:** (emailId, mobileNumber) combination must be unique
...
**Note:** Attempting to update these fields will result in the update being ignored.
